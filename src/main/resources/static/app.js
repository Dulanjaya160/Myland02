...
function initializeApp() {
    ...
    setupFormHandlers();
    setCurrentDate();
    // Load only the initial tab data
...
function loadTabData(tabName) {
    switch(tabName) {
        ...
        case 'sales':
            ...
            // Generate reports after loading sales data
            setTimeout(() => {
...
function displayIngredients(ingredients) {
    ...
    if (ingredients.length === 0) {
        tbody.innerHTML = '<tr><td colspan="4" class="text-center">No ingredients found</td></tr>';
        return;
    }
    ...
    fetch(`${API_BASE}/sale`, {
        ...
        if (!editId) {
            if (confirm('Sale recorded successfully! Do you want to print the bill?')) {
                printSaleBill(data, product, shop);
            }
        }
...
function handleShopSubmit(e) {
    ...
    .catch(error => {
        hideLoading();
        showMessage('Error adding shop: ' + error.message, 'error');
    });
}
